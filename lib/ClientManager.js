...
class ClientManager {
  constructor(opt) {
    ...
    // constants
    this.lowerPortLimit = opt.lowerPortLimit;
    this.upperPortLimit = opt.upperPortLimit;
  ...
  async newClient(id) {
    ...
    let port;
    do {
      port = Math.floor(lpl + Math.random() * (upl - lpl));
    } while (stats.portsEngaged.includes(port));
